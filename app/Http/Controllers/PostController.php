...
final class PostController extends Controller
{
    ...
    public function deneme(CreateCommentRequest $request)
    {
        return config('app.maxdepth');
        $game = Comment::where('id',$request->parent_id)
        ->withDepth()
        ->orderBy('created_at')
        ->first();
        return $game->delete();
...
$comments = Comment::where('id', $request->parent_id)
->withDepth()
->orderBy('created_at')
// ->get()
// ->toTree()
...
//             return $parentComment;
            ...
            return "parent var imiş";
        }




        $felan = $request->parent_id;
        $MyparentComment = Comment::findOrFail($request->parent_id);
        $result = Comment::withDepth()->find($request->parent_id);
        ...
        abort_if($depth === 3, 422, 'Maximum depth can be 3');
        return $depth;
    }
}
